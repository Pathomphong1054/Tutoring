  ...
  Future<void> _fetchMessages() async {
    try {
      final response = await http.get(Uri.parse(
          'http://192.168.92.173/tutoring_app/fetch_chat.php?sender=${widget.currentUser}&recipient=${widget.recipient}'));
  ...
  Future<void> _sendMessage() async {
    ...
    if (message.isNotEmpty) {
      final response = await http.post(
        Uri.parse('http://192.168.92.173/tutoring_app/send_message.php'),
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 'success') {
          ...
          await createNotification(
              widget.currentUser, widget.recipient, message);
        } else {
